...
public class Utils
{
    ...
    {
        if (true) Utils.log("§8[§cHomes§8] §cDebug: §7" + msg.toString());
        debugToFile(msg);
    }
    ...
    {
        File debugFile = new File(Homes.getPlugin().getDataFolder(), "logs/latest.log");
        if (!debugFile.exists())
        {
    ...
    public static void log(String message, int severity)
    {
        ...
        {
            case 1:
                Homes.getPlugin().getLogger().warning(message);
                break;
            case 2:
                Homes.getPlugin().getLogger().severe("! " + message);
                break;
            case 0:
            default:
                Homes.getPlugin().getLogger().info(message);
                break;
        }
    }
    ...
    public static void logToFile(String message)
    {
        try
        {
            File saveTo = new File(Homes.getPlugin().getDataFolder(), "debug.log");
