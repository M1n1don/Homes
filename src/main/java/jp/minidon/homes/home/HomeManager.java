package jp.minidon.homes.home;

import jp.minidon.homes.db.Database;
import jp.minidon.homes.db.SQLQuery;
import jp.minidon.homes.util.Utils;
...
public class HomeManager
{
    ...
    public Home createAndLoadHome(Player player, Location location, String name)
    {
        try
        {
            Database.get().executeStatement(SQLQuery.HOME_INSERT_DEFAULT,
                    name,
    ...
    {
        Database.get().executeStatement(SQLQuery.HOME_DELETE, home.getId());
    }

    private Map<String,List<Home>> selectHomes()
    {
        Map<String, List<Home>> homes = new HashMap<>();
        try (ResultSet rs = Database.get().executeResultStatement(SQLQuery.HOME_SELECT_ALL))
        {
            while (rs.next())
            {
                if (homes.get(rs.getString("uniqueId")) != null && homes.get(rs.getString("uniqueId")).contains(rs.getString("name")))
                    homes.get(rs.getString("uniqueId")).add(getPlayerHome(UUID.fromString(rs.getString("uniqueId")), rs.getString("name")));
                else homes.put(rs.getString("uniqueId"), getPlayerHomeBy(UUID.fromString(rs.getString("uniqueId"))));
            }
        }
        catch (SQLException ex) { Utils.debugSqlException(ex); }
        return homes;
    }
    ...
    {
        Home home = null;
        try (ResultSet rs = Database.get().executeResultStatement(SQLQuery.HOMES_SELECT_ALL_HOME_BY_UUID_NAME,uuid.toString(), name))
        {
            while (rs.next()) home = getHomeFromResultSet(rs);
        }
        catch (SQLException ex) { Utils.debugSqlException(ex); }
        return home;
    }
    ...
    public List<Home> getPlayerHomeBy(UUID uuid)
    {
        ...
        try (ResultSet rs = Database.get().executeResultStatement(SQLQuery.HOMES_SELECT_ALL_HOME_BY_UUID, uuid.toString()))
        {
            while (rs.next())
            {
